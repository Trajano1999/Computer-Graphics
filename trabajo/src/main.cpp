...
void FGE_CambioTamano( GLFWwindow* ventana_glfw, int nuevo_ancho_fb, int nuevo_alto_fb )  // jjj Quitar
{
...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods ) // P3 // P4
{
   using namespace std ;
   ...
   if ( glfwGetKey( window, GLFW_KEY_L) == GLFW_PRESS )
   {
      ...
      ColFuentesLuz * fuentes_luz = escenas[ind_escena_act]->colFuentes();
      revisualizar_escena = ProcesaTeclaFuenteLuz(fuentes_luz, key);
   ...
   switch ( key )
   {
      ...
      // COMPLETAR; Práctica 5. conmutar 'cv.visualizar_fbo' con la tecla 'Y' // P5
       case GLFW_KEY_Y :
         cv.visualizar_fbo = ! cv.visualizar_fbo ;
         cout << "Visualización del FBO : " << (cv.visualizar_fbo ? "activado" : "desactivado") << endl << flush ;
         break ;
...
void FGE_PulsarLevantarBotonRaton( GLFWwindow* window, int button, int action, int mods ) // P5
{
...
void Inicializar( int argc, char *argv[] )   // P2 // P3
{
