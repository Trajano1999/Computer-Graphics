...
void Camara3Modos::desplRotarXY( const float da, const float db ) // P5
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         const float da_s = da/50.0;
         const float db_s = db/50.0;
         org_polares = org_polares + Tupla3f({ da_s, db_s, 0.0 });
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         Tupla3f cartesianas_previas = org_cartesianas;
         org_cartesianas = Cartesianas(org_polares);

         Tupla3f desplazamiento = cartesianas_previas - org_cartesianas;
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         // jjj * 0.25
         Tupla3f desplazamiento = da*eje[0]*0.25 + db*eje[1]*0.25;
         punto_atencion = punto_atencion + desplazamiento;
...
void Camara3Modos::moverZ( const float dz )  // P5
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         const float r_min = 0.5, epsilon = 0.1; 
         float radio = r_min + (org_polares(2) - r_min)*pow( 1.0 + epsilon, dz);
         org_polares = { org_polares(0), org_polares(1), radio };
         org_cartesianas = Cartesianas(org_polares);
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         // jjj * 0.25
         punto_atencion = punto_atencion + dz*eje[Z]*0.25;
...
void Camara3Modos::mirarHacia( const Tupla3f & nuevo_punto_aten ) // P5
{
   // COMPLETAR: práctica 5
