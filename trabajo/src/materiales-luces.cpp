...
Textura::Textura( const std::string & nombreArchivoJPG ) // P4
{
...
void Textura::enviar()  // P4
{
   // COMPLETAR: práctica 4: enviar la imagen de textura a la GPU
   // y configurar parámetros de la textura (glTexParameter)

   glGenTextures( 1, &ident_textura );                                                             // creamos el identificador
   glActiveTexture(GL_TEXTURE0);                                                                   // activamos la textura con el identificador 0
   glBindTexture(GL_TEXTURE_2D, ident_textura);                                                    // activamos la textura con el identificador indicado
...
void Textura::activar( Cauce & cauce  )   // P4
{
...
TexturaXY::TexturaXY( const std::string & nom ) : Textura(nom) {   // P4 
   modo_gen_ct = mgct_coords_objeto;
}

TexturaXZ::TexturaXZ( const std::string & nom ) : Textura(nom) {   // P4
   coefs_t[1] = 0.0;
   coefs_t[2] = 1.0;
   modo_gen_ct = mgct_coords_objeto;
}
...
void Material::activar( ContextoVis & cv )   // P4
{
...
void ColFuentesLuz::activar( Cauce & cauce ) // P4
{
