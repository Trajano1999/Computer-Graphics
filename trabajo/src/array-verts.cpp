...
void DescrTabla::fijarPuntero( const GLvoid * ptr_offset )  // P1
{
   using namespace std ;
   ...
   // especificamos la localización y el formato de las tablas de los atributos en el VAO
   switch (atributo){                                                          
...
void DescrTabla::activar_md() // P1
{
   ...
   // comprobamos que el VBO no está creado todavía
   if (nombre_vbo == 0){                                                       
      glGenBuffers(1, &nombre_vbo);                                                                // creamos el VBO                                           
      glBindBuffer(tipo_tabla, nombre_vbo);                                                        // activamos el buffer VBO creado
      glBufferData(tipo_tabla, tamano_en_bytes, datos, GL_STATIC_DRAW);                            // reservamos memoria en la GPU y tranferimos bytes de la tabla                                    
   }else
      glBindBuffer(tipo_tabla, nombre_vbo);                                                        // activamos el buffer VBO ya existente

   // comprobamos si es tabla de atributos
   if (tipo_tabla == GL_ARRAY_BUFFER){                                         
      fijarPuntero(0);                                                                             // especificamos loc. y formato        
      glBindBuffer(tipo_tabla, 0);                                                                 // desactivamos el buffer VBO                                               
      glEnableClientState(atributo);                                                               // habilitamos el uso de la tabla
   }
...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva )  // P1
{
   CError();
   ...
   const GLuint nv = (indices != nullptr) ? indices->num_tuplas_ind : num_vertices;
   
   // delimitamos los vértices de la primitiva
   glBegin(tipo_primitiva);                                                    

   // recorremos los vértices
   for(GLuint i=0; i<nv; ++i){    
      // recuper el índice de vértice en la tabla de coordenadas (iv)                                             
...
void ArrayVertices::visualizarGL_MI_DAE( const GLenum tipo_primitiva )  // P1
{
   using namespace std ;
   ...
   // * se debe activar el VAO por defecto (VAO 0) al inicio
   // * se deben de deshabilitar todas las tablas, al inicio y al final.
   
   // activamos VAO por defecto
   glBindVertexArray(0);   

   // deshabilitamos todas la tablas                                                    
   deshabilitar_tablas();     

   // activamos la tabla de coordenadas                                                 
...
void ArrayVertices::visualizarGL_MD_VAO( const GLenum tipo_primitiva )  // P1
{
   using namespace std ;
   //cout << __FUNCTION__ << " : inicio " << endl ;
   CError();
   ...
   // si el VAO no estaba creado
   if ( nombre_vao == 0 ){                                
      glGenVertexArrays(1, &nombre_vao);                                                           // creamos el nuevo VAO                                      
      glBindVertexArray(nombre_vao);                                                               // activamos el nuevo VAO
      coordenadas->activar_md();                                                                   // activar las tablas que proceda (incluyendo índices)
   ...
   // si el VAO ya estaba creado
   else                                                                        
      glBindVertexArray(nombre_vao);                                                               // activamos el VAO                                           
